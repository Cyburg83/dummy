...
void CAbase::_updateView(char *&grid_view, int *grid, int Nx, int Ny) {
    std::cout << "_updateView called" << std::endl;
...
void CAbase::_printGrid(char* grid, int size_x, int size_y) {
    std::cout << "_printGrid called" << std::endl;
    //std::cout << std::string(2*size_x + 1, '.' + ' ') + "." << std::endl;
    ...
    std::cout << "." << std::endl;
    //std::string(2*size_x + 1, '.' + ' ') + "." << std::endl;
    for (int i = 0; i <= size_x+1; i++) {
        std::cout << ". ";
    }
}

void CAbase::_resizeGrid(int* &grid, char* &grid_view, int Nx, int Ny) {
...
void CAbase::showCurrent() {
    _updateView(_currentView, _current, _size_x, _size_y);
    _printGrid(_currentView, _size_x, _size_y);
}

void CAbase::showNext() {
    _updateView(_nextView, _next, _size_x, _size_y);
    _printGrid(_nextView, _size_x, _size_y);
}
